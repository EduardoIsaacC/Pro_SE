...
def evaluar_pedido(articulos_solicitados):
    ...
    # Usamos la ruta absoluta para que no se pierda la base de datos
    ruta_bd = os.path.join(os.path.dirname(__file__), 'aderezos_edu.db')
    conexion = sqlite3.connect(ruta_bd)
    cursor = conexion.cursor()

    # Variables de control e información del reporte (¡Indispensables aquí!)
    ...
    if cantidad_total_botellas >= 5:
        ...
        alertas.append("Inferencia comercial: Se aplicó un 10% de descuento por llevar 5 unidades")
...
if __name__ == "__main__":
    ...
    print(f"Estado general: {resultado['estado']}")
